...
class ProgressManager {
	...
	class func checkAndSetAchievementReached(view: UIViewController, type: String) {
		...
		else if type == Constants.Achievements.PEOPLE_TYPE {
			for achievement in Constants.Achievements.ACHIEVEMENTS_PEOPLE {
				let value = Int(achievement[0].components(separatedBy: "pp")[0])
				if Utils.int(key: Constants.Defaults.APP_DATA_TOTAL_PEOPLE_MET) >= value! {
					checkAchivementReached(view: view, achievement: achievement)
				}
			}
		}
		else if type == Constants.Achievements.CATCHUPS_TYPE {
			for achievement in Constants.Achievements.ACHIEVEMENTS_CATCHUPS {
				let value = Int(achievement[0].components(separatedBy: "c")[0])
				if Utils.int(key: Constants.Defaults.APP_DATA_TOTAL_CATCHUPS_COMPLETED) >= value! {
					checkAchivementReached(view: view, achievement: achievement)
				}
			}
		}
