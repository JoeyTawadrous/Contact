...
class Dialogs {
	...
	class func showInfoDialog(title: String, subTitle: String) {
		...
		alertView.showCustom(title, subTitle: subTitle, color: UIColor(hex: Constants.Colors.GREEN), icon: alertViewIcon!, animationStyle: .rightToLeft)
	}
	...
	class func showOkButtonDialog(view: UIViewController, message: String) {
		...
		let alertView = SCLAlertView(appearance: appearance)
		let alertViewIcon = UIImage(named: "leaf")
