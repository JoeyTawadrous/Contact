...
//  ContactWidgets
//
//  Created by Kaushik Gadhiya on 08/12/18.
...
class TodayViewController: UIViewController, NCWidgetProviding, UITableViewDelegate, UITableViewDataSource {
    ...
    override func viewDidLoad() {
        ...
        let catchups = self.fetchTodayCatchups(Constants.CoreData.CATCHUP, startDate: Date().startOfDay, endDate: Date().endOfDay)
        
        for catchup in catchups
        {
            let name = catchup.value(forKey: Constants.CoreData.NAME) as! String?
            people.append(self.fetchPeople(Constants.CoreData.PERSON, name: name!)[0])
        }
        
        self.people = people.reversed() // newest first
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell
    {
        if people.count == 0
        {
            ...
            if cell == nil {
                cell = PeopleTableViewCell(style: UITableViewCellStyle.value1, reuseIdentifier: "noCell")
            }
            return cell
        }
        else
        {
            ...
            let catchups = self.fetchCatchupsCount(Constants.CoreData.CATCHUP, name: name!, startDate: Date().startOfDay, endDate: Date().endOfDay)
...
class CoreDataStack {
   ...
   static var persistentStoreCoordinator: NSPersistentStoreCoordinator? = {
        ...
        let url = CoreDataStack.secureAppGroupPersistentStoreURL.appendingPathComponent("Contact.sqlite")
